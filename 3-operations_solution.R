...
popul.tidy |>
    summarise(.by = year, total = sum(pop))
...
pp |>
    summarise(.by = c(gender, institution), mean_size = mean(size))
...
tib <- read_csv(
    flist, # what do we want to read? give the vector of file names
    id = 'file'
...
tib <- tib |>
    separate(col = file, into = c('sample', 'T', 'time', NA)) |>
    mutate(
        sample = sample |> str_remove('sample') |> as.numeric(),
